...
def run_fasta_to_darwin(root_genome_fp,
	...
	"""
	with open(root_genome_db_fp, 'w') as root_genome_db_f:
		with open(root_genome_fp, 'U') as root_genome_f:
			proc = Popen("fasta2darwin",
					 	stdin=root_genome_f,
					 	stdout=root_genome_db_f,
					 	stderr=PIPE,
					 	close_fds=True)
			proc.wait()
